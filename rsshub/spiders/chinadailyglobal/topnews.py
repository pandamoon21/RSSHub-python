...
domain = 'https://www.chinadailyglobal.com'

# 默认抓取页面:China Daily Global(国际版) Top News(头条)列表
# https://www.chinadailyglobal.com/e/5c00a33ba310eff30328c087
DEFAULT_SECTION = 'e/5c00a33ba310eff30328c087'

# 抓取预算(秒):页面约 110KB,海外机房通常 1~4s 可完成。
REQUEST_BUDGET = 7.0
...
def ctx(section=DEFAULT_SECTION):
    ...
    # 频道名取自页面 <title>: 如 "Top News - Chinadaily.com.cn"
    channel = 'China Daily Global'
    if tree.title:
    ...
    if not items:
        raise RuntimeError(f'China Daily Global 列表解析为空(页面结构可能已变更): {url}')
